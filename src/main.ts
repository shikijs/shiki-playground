...
import { createPlaygroundStore, State } from './store'
...
const settingsFromURL = () => {
  ...
  if (location.hash.startsWith('#code')) {
    ...
    if (!userCode) userCode = lzstring.decompressFromEncodedURIComponent(decodeURIComponent(codeHash))
    code = userCode || ' '
  }
...
initHighlighter().then(() => {
  ...
  createApp(App).use(createPlaygroundStore(settings)).mount('#app')
})
